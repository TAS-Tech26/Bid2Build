...
from .models import AssetPurchase, AuctionParticipant, BidLog, Team, Technology
...
def fetch_credits(request):
    try:
        ...
        return JsonResponse({'error': 'Team not found in Bid2Build system. Please sync wallets.'}, status=404)
...
def start_auction_view(request, tech_id):
    ...
    if getattr(request.user, 'role', 'participant') != 'admin':
    ...
    try:
        with transaction.atomic():
            ...
            # Enforce only ONE active auction across the platform at a time
            if Technology.objects.filter(status = 'ACTIVE').exists():

                return JsonResponse({'error': "Another auction is already active. Please settle it first."}, status = 400)
...
def push_final_results_view(request):
    ...
    for rank, team in enumerate(teams, start = 1):
        ...
        results.append({'rank': rank, 'team_code': team.team_code, 'assets': assets_won})
...
@authentication_classes([HubJWTAuthentication])
@permission_classes([IsAuthenticated])
def trigger_disruption_view(request):
    """Admin endpoint to trigger a global market disruption event."""
    ...
    disruption_data = {
        'title': request.data.get('title', "Market Disruption"),
        'description': request.data.get('description', ''),
        'expected_adaptation': request.data.get('expected_adaptation', ''),
        'affected_teams': request.data.get('affected_teams', 'ALL')
    }
    RedisService.broadcast_market_disruption(disruption_data)
    
    return JsonResponse({'message': 'Disruption broadcasted successfully'}, status = 200)
